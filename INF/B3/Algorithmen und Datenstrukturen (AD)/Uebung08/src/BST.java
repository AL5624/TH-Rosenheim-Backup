...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    public Iterable<Key> keysPreOrderIterative() {
        ...
        while(keys.size() != this.size())
        {
            ...
            {
                tmp = current.parent;
                while(tmp.right == null || tmp.right.equals(current))
                {
                    if(tmp.right == null)current = tmp;
                    tmp = tmp.parent;
                }
       ...
       // keys = keysPreOrderIterative(root);

        return keys;
    }

    private List<Key> keysPreOrderIterative(Node node)
    {
        List<Key> keys = new LinkedList<>();

        if(node != null)keys.add(node.key);
        if(node.left != null) keys.addAll(keysPreOrderIterative(node.left));
        if(node.right != null) keys.addAll(keysPreOrderIterative(node.right));
        return keys;

...
with 
	-- Als erstes bauen wir uns die Werte für 80%, 95% und 100% des Umsatzes zusammen
	ABC_Punkte(A,B,C) as(
	...
	-- Dann müssen wir wissen, wieviel Umsatz jeder Kunde für sich generiert hat
	...
	-- Jetzt können wir eine ABC Analyse machen
	ABC_Kunden (kunde, umsatz, kumuliert, typ) as(
		select	K.kunde, -- Wir nehemn den Kunden 
				K.umsatz, -- und seinen Umsatz
				(Select sum(V.umsatz) from Kundenumsatz as V where V.umsatz>=K.umsatz), -- dann die Summe der Umsätze von Kunden die mehr ausgegeben haben
				case 
